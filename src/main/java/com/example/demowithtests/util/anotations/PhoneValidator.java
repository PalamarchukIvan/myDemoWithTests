...
public class PhoneValidator implements ConstraintValidator<Phone, String> {
    ...
    public boolean isValid(String s, ConstraintValidatorContext constraintValidatorContext) {
        ...
        switch (from) {
            case UKRAINE: return s.startsWith("+38") && s.length() == 13;

            case POLAND: return s.startsWith("+48") && s.length() == 13;

            case GERMANY: return s.startsWith("+49") && s.length() == 13;

            case ANY: return s.startsWith("+") && (s.length() > 7 && s.length() < 16);
